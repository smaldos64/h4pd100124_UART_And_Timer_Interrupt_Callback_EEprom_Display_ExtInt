...
static volatile ExternalInterruptCallbackStruct ExternalInterruptCallbackStructArray[EXTERNAL_INTERRUPT_7 + 1] =
{
...
bool AttachExternalInterrupt(uint8_t ExternalInterruptNumber, Function_Pointer_With_One_Uint16t_Parameter FunctionPointer, Interrupt_Cause_C InterruptMode)
{
	if (ExternalInterruptNumber <= EXTERNAL_INTERRUPT_1)
	{
...
bool DetachExternalInterrupt(uint8_t ExternalInterruptNumber)
{
	if (ExternalInterruptNumber <= EXTERNAL_INTERRUPT_1)
	{
		if (!(NoCallbackIsPerformedOnExternalInterrupt == ExternalInterruptCallbackStructArray[ExternalInterruptNumber].ThisFunctionPointer))
